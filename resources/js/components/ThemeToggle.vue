...
<template>
  ...
  >
    ...
    >
      ...
      <span
        class="ml-1 max-w-0 overflow-hidden whitespace-nowrap opacity-0 transition-all duration-200 group-hover:max-w-[64px] group-hover:opacity-100 focus:opacity-100 focus:max-w-[64px]"
      >{{ mode.label }}</span>
    </button>
  </div>
</template>
...
button + button { border-left: 1px solid hsl(var(--border)); }
</style>
